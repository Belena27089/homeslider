...
<li>Двигатель: 300 - 1300 W</li>
                ...
                $slide->icon[$language['id_lang']] = 'sample1-' . $i . '.jpg';
    ...
    protected function _postValidation() {
        ...
        elseif (Tools::isSubmit('submitSlide')) {
            ...
            foreach ($languages as $language) {
                ...
                if (Tools::strlen(Tools::getValue('description_short_' . $language['id_lang'])) > 4000)
                    $errors[] = $this->l('The descriptio_short is too long.');
                if (Tools::strlen(Tools::getValue('url_' . $language['id_lang'])) > 0 && !Validate::isUrl(Tools::getValue('url_' . $language['id_lang'])))
                    $errors[] = $this->l('The URL format is not correct.');
                if (Tools::getValue('icon_' . $language['id_lang']) != null && !Validate::isFileName(Tools::getValue('icon_' . $language['id_lang'])))
                    $errors[] = $this->l('Invalid filename.');
                if (Tools::getValue('icon_old_' . $language['id_lang']) != null && !Validate::isFileName(Tools::getValue('icon_old_' . $language['id_lang'])))
                    $errors[] = $this->l('Invalid filename.');
            ...
            if (Tools::getValue('image_old_' . $id_lang_default) && !Validate::isFileName(Tools::getValue('image_old_' . $id_lang_default)))
                $errors[] = $this->l('The image is not set.');
            if (!Tools::isSubmit('has_picture') && (!isset($_FILES['icon_' . $id_lang_default]) || empty($_FILES['icon_' . $id_lang_default]['tmp_name'])))
                $errors[] = $this->l('The icon is not set.');
            if (Tools::getValue('icon_old_' . $id_lang_default) && !Validate::isFileName(Tools::getValue('icon_old_' . $id_lang_default)))
                $errors[] = $this->l('The icon is not set.');
    ...
    protected function _postProcess() {
        ...
        elseif (Tools::isSubmit('submitSlide')) {
            ...
            foreach ($languages as $language) {
                ...
                $slide->description_short[$language['id_lang']] = Tools::getValue('description_short_' . $language['id_lang']);

                /* Uploads icon and sets slide */
                $type = Tools::strtolower(Tools::substr(strrchr($_FILES['icon_' . $language['id_lang']]['name'], '.'), 1));
                $imagesize = @getimagesize($_FILES['icon_' . $language['id_lang']]['tmp_name']);
                if (isset($_FILES['icon_' . $language['id_lang']]) &&
                        isset($_FILES['icon_' . $language['id_lang']]['tmp_name']) &&
                        !empty($_FILES['icon_' . $language['id_lang']]['tmp_name']) &&
                        !empty($imagesize) &&
                        in_array(
                    ...
                    $salt = sha1(microtime());
                    if ($error = ImageManager::validateUpload($_FILES['icon_' . $language['id_lang']]))
                        $errors[] = $error;
                    elseif (!$temp_name || !move_uploaded_file($_FILES['icon_' . $language['id_lang']]['tmp_name'], $temp_name))
                        return false;
                    elseif (!ImageManager::resize($temp_name, dirname(__FILE__) . '/images/' . $salt . '_' . $_FILES['icon_' . $language['id_lang']]['name'], null, null, $type))
                        $errors[] = $this->displayError($this->l('An error occurred during the icon upload process.'));
                    if (isset($temp_name))
                        @unlink($temp_name);
                    $slide->icon[$language['id_lang']] = $salt . '_' . $_FILES['icon_' . $language['id_lang']]['name'];
                }
                elseif (Tools::getValue('icon_old_' . $language['id_lang']) != '')
                    $slide->icon[$language['id_lang']] = Tools::getValue('icon_old_' . $language['id_lang']);
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null) {
        ...
        $images[$result['id_lang']] = $result['icon'];
    ...
    public function renderList() {
        ...
        $this->context->smarty->assign(
                array(
                    ...
                    'icon_baseurl' => $this->_path . 'images/'
                )
        );
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    array(
                        'type' => 'file_lang',
                        'label' => $this->l('Select a file for icon'),
                        'name' => 'icon',
                        'required' => true,
                        'lang' => true,
                    ...
                    array(
                        'type' => 'file_lang',
                        'label' => $this->l('Select a file for banner'),
                        'name' => 'image',
                        'required' => true,
                        'lang' => true,
                        'desc' => sprintf($this->l('Maximum image size: %s.'), ini_get('upload_max_filesize'))
                    ...
                    array(
                        'type' => 'textarea',
                        'label' => $this->l('Description left edge'),
                        'name' => 'description',
                        'autoload_rte' => true,
                        'lang' => true,
                    ),
                    array(
                        'type' => 'textarea',
                        'label' => $this->l('Description right edge'),
        ...
        if (Tools::isSubmit('id_slide') && $this->slideExists((int) Tools::getValue('id_slide'))) {
            ...
            foreach (Language::getLanguages(false) as $lang)
                if (!isset($slide->image[$lang['id_lang']]))
                    $has_picture &= false;
            if (!isset($slide->icon[$lang['id_lang']]))
                $has_picture &= false;
    ...
    public function getAddFieldsValues() {
        ...
        foreach ($languages as $lang) {
            $fields['icon'][$lang['id_lang']] = Tools::getValue('icon_' . (int) $lang['id_lang']);
            ...
            $fields['description_short'][$lang['id_lang']] = Tools::getValue('description_short_' . (int) $lang['id_lang'], $slide->description_short[$lang['id_lang']]);
        }
