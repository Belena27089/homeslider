...
class PwHomeBanners extends ObjectModel
{
	public $title;
	public $description;
	...
	public static $definition = array(
		'table' => 'pwhomebanner_slides',
		'primary' => 'id_pwhomebanner_slides',
		'multilang' => true,
		'fields' => array(
			...
			'description_short' =>	array('type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isCleanHtml', 'size' => 4000),
			...
			'icon' =>			array('type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isCleanHtml', 'size' => 255),
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'pwhomebanner` (`id_shop`, `id_pwhomebanner_slides`)
	...
	public function delete()
	{
		...
		foreach ($icons as $icon)
		{
			if (preg_match('/sample/', $icon) === 0)
				if ($icon && file_exists(dirname(__FILE__).'/images/'.$icon))
					$res &= @unlink(dirname(__FILE__).'/images/'.$icon);
		}

		$res &= $this->reOrderPositions();

		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'pwhomebanner`
			WHERE `id_pwhomebanner_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_pwhomebanner_slides` = hs.`id_pwhomebanner_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_pwhomebanner_slides` as id_slide
			FROM `'._DB_PREFIX_.'pwhomebanner_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'pwhomebanner` hs ON (hss.`id_pwhomebanner_slides` = hs.`id_pwhomebanner_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new PwHomeBanners($row['id_slide']);
